...
const DICT = {
  en: {
    ...
    'tool.predictions': 'FORECAST',
    'tool.predictions.title': 'Prediction Engine — standing forecasts with a deadline, a probability and a scoreboard that records the misses',
    'tool.chat': 'CHAT',
    ...
    'reasoning.collapse': 'Collapse to side panel',
    'reasoning.expand': 'Expand to full screen',
    'reasoning.close': 'Close',
    'reasoning.mode.all': 'ALL',
    'reasoning.mode.all.title': "The entire graph, not one node's neighbourhood",
    'reasoning.search.placeholder': (n: string) => `Search ${n} nodes...`,
    'reasoning.search.placeholder.empty': 'Search a node...',
    'reasoning.network.loading': 'Loading the whole network... the first request trains the model, which takes a moment.',
    'reasoning.embeddings.loading': 'Loading graph embeddings...',
    'reasoning.network.hint': 'drag = rotate · scroll = zoom · double-click = reset · click a node to jump',
    'reasoning.network.stats': (nodes: string, edges: string) => `${nodes} nodes · ${edges} edges · position = PCA of the learned embedding`,
    'reasoning.provenance.observed.title': 'Came out of an audit finding',
    'reasoning.provenance.derived.title': 'Computed from a real field, not observed',
    'reasoning.type.hide': (t: string) => `Hide ${t}`,
    'reasoning.type.show': (t: string) => `Show ${t}`,
    'reasoning.uncoloured.marker.title': 'no colour assigned',
    'reasoning.uncoloured': '* uncoloured',
    'reasoning.uncoloured.title': 'These node types render grey because NetworkView has no colour for them',
    'reasoning.showAll': 'show all',
    'reasoning.neighbours.count': (n: string) => `${n} nearest neighbours`,
    'reasoning.neighbours.fallback': 'Neighbours',
    // Reasoning feed
    ...
    // Prediction engine
    'pred.title': 'PREDICTION ENGINE',
    'pred.loading': 'reading the forecast store...',
    'pred.empty': 'no forecast has been issued yet.',
    'pred.gated': 'sign in to see standing forecasts.',
    'pred.atleastone': 'chance of at least one event in the window',
    'pred.expected': 'expected',
    'pred.sequences': 'sequences',
    'pred.observed': 'observed so far',
    'pred.open': 'window open',
    'pred.closed': 'window closed',
    'pred.chain.show': 'show the causality chain',
    'pred.chain.hide': 'hide the causality chain',
    // Native tooltips render at the cursor and get clipped at the viewport
    // edge, which is exactly where this panel lives. Kept short enough to fit.
    'pred.title.hint': 'Whitebox for this earthquake',
    'pred.body.hint': 'Open the causality chain',
    'pred.chain.title': 'CAUSALITY CHAIN',
    'pred.chain.close': 'Close (Esc)',
    // WAS "coverage sits near nominal, the intervals are mostly getting it
    // right" until that was measured and turned out to be false. Coverage does
    // match nominal, and it is an artifact: it is carried by cases predicting
    // under 5 events, where rounding an interval onto whole events makes it
    // ~1.9x wider than +-1 sd. Split by how much is actually claimed:
    // 17/17 covered below 5 predicted, 1/3 at or above it.
    'pred.calibrated': 'Coverage matches nominal — but measured, that is an artifact of integer intervals on small counts. Where the forecast asserts 5+ events it lands 1 time in 3. Read these percentages as upper bounds.',
    'pred.overcovered': 'Over-covered: almost everything lands inside the interval, which means the interval is too wide to assert much. Not a good score.',
    'pred.undercovered': 'Under-covered: reality falls outside the stated interval too often. The uncertainty is understated.',
    'pred.skipped': 'malformed lines skipped while reading the store',
    // Evidence panel
  ...
  de: {
    ...
    'tool.predictions': 'PROGNOSE',
    'tool.predictions.title': 'Prognose-Engine — laufende Vorhersagen mit Frist, Wahrscheinlichkeit und einer Bilanz, die auch die Fehlschläge festhält',
    'tool.chat': 'CHAT',
    ...
    'reasoning.collapse': 'Auf Seitenpanel verkleinern',
    'reasoning.expand': 'Auf Vollbild erweitern',
    'reasoning.close': 'Schließen',
    'reasoning.mode.all': 'ALLE',
    'reasoning.mode.all.title': 'Der gesamte Graph, nicht nur die Nachbarschaft eines Knotens',
    'reasoning.search.placeholder': (n: string) => `${n} Knoten durchsuchen...`,
    'reasoning.search.placeholder.empty': 'Einen Knoten suchen...',
    'reasoning.network.loading': 'Lade das gesamte Netzwerk … die erste Anfrage trainiert das Modell, das dauert einen Moment.',
    'reasoning.embeddings.loading': 'Lade Graph-Embeddings...',
    'reasoning.network.hint': 'ziehen = drehen · scrollen = zoomen · Doppelklick = zurücksetzen · Knoten anklicken = springen',
    'reasoning.network.stats': (nodes: string, edges: string) => `${nodes} Knoten · ${edges} Kanten · Position = PCA des gelernten Embeddings`,
    'reasoning.provenance.observed.title': 'Stammt aus einem Audit-Befund',
    'reasoning.provenance.derived.title': 'Aus einem realen Feld berechnet, nicht beobachtet',
    'reasoning.type.hide': (t: string) => `${t} ausblenden`,
    'reasoning.type.show': (t: string) => `${t} einblenden`,
    'reasoning.uncoloured.marker.title': 'keine Farbe zugewiesen',
    'reasoning.uncoloured': '* ungefärbt',
    'reasoning.uncoloured.title': 'Diese Knotentypen werden grau dargestellt, weil NetworkView keine Farbe für sie kennt',
    'reasoning.showAll': 'alle anzeigen',
    'reasoning.neighbours.count': (n: string) => `${n} nächste Nachbarn`,
    'reasoning.neighbours.fallback': 'Nachbarn',
    'feed.title': 'REASONING-FEED',
    ...
    // Prognose-Engine
    'pred.title': 'PROGNOSE-ENGINE',
    'pred.loading': 'lese den Prognosespeicher...',
    'pred.empty': 'es wurde noch keine Prognose ausgegeben.',
    'pred.gated': 'anmelden, um laufende Prognosen zu sehen.',
    'pred.atleastone': 'Wahrscheinlichkeit für mindestens ein Ereignis im Fenster',
    'pred.expected': 'erwartet',
    'pred.sequences': 'Sequenzen',
    'pred.observed': 'bisher beobachtet',
    'pred.open': 'Fenster offen',
    'pred.closed': 'Fenster geschlossen',
    'pred.chain.show': 'Kausalkette anzeigen',
    'pred.chain.hide': 'Kausalkette ausblenden',
    'pred.title.hint': 'Whitebox zu diesem Beben',
    'pred.body.hint': 'Kausalkette öffnen',
    'pred.chain.title': 'KAUSALKETTE',
    'pred.chain.close': 'Schließen (Esc)',
    'pred.calibrated': 'Die Abdeckung trifft den Sollwert — nachgemessen ist das jedoch ein Artefakt ganzzahliger Intervalle bei kleinen Anzahlen. Wo die Prognose 5+ Ereignisse behauptet, trifft sie in einem von drei Fällen. Diese Prozentwerte sind obere Schranken.',
    'pred.overcovered': 'Überabgedeckt: fast alles landet im Intervall, das Intervall ist also zu breit, um etwas zu behaupten. Kein gutes Ergebnis.',
    'pred.undercovered': 'Unterabgedeckt: die Realität fällt zu oft aus dem angegebenen Intervall. Die Unsicherheit ist zu klein angesetzt.',
    'pred.skipped': 'fehlerhafte Zeilen beim Lesen des Speichers übersprungen',
