...
// Uint8Array became generic over its backing buffer in TS 5.7, and BufferSource
// admits ArrayBuffer but not SharedArrayBuffer -- so a bare Uint8Array no longer
// satisfies it. Naming the buffer type says which one this is instead of casting
// the distinction away.
async function writeSafe(ch:BluetoothRemoteGATTCharacteristic,d:Uint8Array<ArrayBuffer>,resp:boolean){if(resp&&typeof ch.writeValueWithResponse==='function')await ch.writeValueWithResponse(d);else if(!resp&&typeof ch.writeValueWithoutResponse==='function')await ch.writeValueWithoutResponse(d);else await(ch as any).writeValue(d);}
